...
* **Tuba (PM/Design):**
  ...
  * *Engel (Blocker):* Yok.
* **Kübra (Backend):**
  * *Bugün Yapılanlar:* Auth, Products, Users ve Addresses modüllerine ait tüm endpoint'ler (profil görüntüleme, adres ekleme/güncelleme/silme sahiplik kontrolleri dahil) kodlandı ve gerçek isteklerle (400, 401, 403, 404 senaryoları) test edildi.
  * *Engel (Blocker):* Yok.
* **İkram (Frontend/QA):**
  * *Bugün Yapılanlar:* Full Auth kısmı API entegrasyonu ile test edilip çalışır hale getirildi. Playwright testleri genişletildi. `persistAuthSession()` fonksiyonu ile cookie silinme sorunu çözüldü, 401 hatası için merkezi yönlendirme eklendi. Admin ve kullanıcı girişleri için ayrıştırılmış cookie yapılarına geçildi ve cart yapısı kuruldu.
  * *Engel (Blocker):* Yok.
* **Zeliha (Mobil):**
  * *Bugün Yapılanlar:* MOB-4 kapsamında FlutterSecureStorage ile JWT token yönetimi tamamlandı; Dio Interceptor kurularak istekler otomatik Bearer token'a bağlandı ve 401 durumunda oturum kapatma kurgulandı. Profil ekranı dinamik hale getirildi. MOB-5 kapsamında sepet işlemleri (ekle/çıkar/güncelle) backend ile uyumlu hale getirilerek anlık toplam fiyat güncellemeleri sağlandı.
  * *Engel (Blocker):* Yok.
